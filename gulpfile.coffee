...
gulp.task 'bundle', ->
  ...
  )).pipe(source('bundle.js')).pipe(buffer()).pipe(sourcemaps.init(
    loadMaps: true
    debug: true)).pipe(uglify()).pipe(sourcemaps.write('./maps')).pipe(gulp.dest('./public/js/')).pipe sync.stream()

gulp.task 'stylus', ->
  gulp.src(dirs.stylus + '/main.styl').pipe(sourcemaps.init()).pipe(stylus(rawDefine: config: config).on('error', notify.onError((error) ->
    {
      title: 'Stylus error: ' + error.name
      message: error.message
      sound: 'Pop'
    }
  ))).pipe(sourcemaps.write()).pipe(gulp.dest('public/css')).pipe sync.stream()
  return
...
gulp.task 'coffee', [ 'objectus', 'coffee', 'stylus', 'pug', 'vendor' ]
